...
final class FW_SocialMedia_Twitter{
	...
	private static $consumerKey = 'cd9osJg6G3ZKXVwFRt1Q';
	private static $consumerSecret = 'WbZMnpIajOLIvqXUDo1NSSjB4a9jZi076JLYW5lIIY';
	private static $accessToken = '740904212-mQeGOQ1mWZuLjkiyrmVvaQtSDyDxDzQtm0vPzsCo';
	private static $accessTokenSecret = 'W4RBVlRZAyBaSkM6USAqy13oWsGhRAV4m4abQLnvB34CP';
	private static $requestTokenUrl = 'request_token';
	private static $authorizeUrl = 'authorize';
	private static $accessTokenUrl = 'access_token';
	...
	public function __construct(){
		if(!extension_loaded('curl')) {
            throw new FW_Exception_SocialMedia('PHP extension CURL is not loaded.');
        }

        $this->signatureMethod = new FW_SocialMedia_Twitteroauth_OAuthSignatureMethodHMACSHA1();
        $this->consumer = new FW_SocialMedia_Twitteroauth_OAuthConsumer(self::$consumerKey, self::$consumerSecret);
        $this->token = new FW_SocialMedia_Twitteroauth_OAuthConsumer(self::$accessToken, self::$accessTokenSecret);
	...
	public function authenticate()
        {
                ...
                } catch (FW_Exception_SocialMedia $e) {
                        if ($e->getCode() === 401) {
                                return FALSE;
                        }
                        throw $e;
        ...
        public function request($resource, $method, array $data = NULL)
        {
                ...
                $request = FW_SocialMedia_Twitteroauth_OAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $resource, $data);
                ...
                if (curl_errno($curl)) {
                        throw new FW_Exception_SocialMedia('Server error: ' . curl_error($curl));
                }
                ...
                if ($payload === FALSE) {
                        throw new FW_Exception_SocialMedia('Invalid server response');
                }

                $code = curl_getinfo($curl, CURLINFO_HTTP_CODE);
                if ($code >= 400) {
                        throw new FW_Exception_SocialMedia(isset($payload->errors[0]->message) ? $payload->errors[0]->message : "Server error #$code", $code);
                }
